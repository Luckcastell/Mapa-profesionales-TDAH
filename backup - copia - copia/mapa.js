...
function aCSV(filas) {
  const cabeceras = ["id","nombre","esCentro","tipoCentro","especialidades","poblacion","lat","lng","direccion","ciudad","pais","zona","web","contacto"];
  ...
  const lineas = [cabeceras.join(",")].concat(
    rows.map((r) => cabeceras.map((h) => {
      const v = (r[h] ?? "").toString().replace(/"/g, '""');
      const necesitaComillas = v.includes(",") || v.includes("\n") || v.includes('"') || v.includes(";");
      return necesitaComillas ? `"${v}"` : v;
    }).join(","))
  );
  return lineas.join("\n");
}
...
function pintarMarcadores(filas) {
  ...
  filas.forEach((it) => {
    ...
    const web = it.web ? `<div><a href="${it.web}" target="_blank" rel="noreferrer">Sitio web</a></div>` : "";
    const contacto = it.contacto ? `<div>Contacto: ${it.contacto}</div>` : "";
    m.bindPopup(`
      <div class="popup">
...
function prepararExportarImportar() {
  ...
  el.btnExportarCSV.addEventListener("click", () => descargar("profesionales.csv", aCSV(datos), "text/csv"));
...
function renderizarLeyenda() {
  el.leyenda.innerHTML = `
    ...
    <div class="item"><span class="muestra" style="background:${COLORES_CATEGORIA["centro de integracion"]}"></span> Centro de integración</div>
    <div class="item"><span class="muestra" style="background:${COLORES_CATEGORIA["centro clinica de profecionales"]}"></span> Centro clínica de profecionales</div>
    <div class="item"><span class="muestra" style="background:${COLORES_CATEGORIA["centro de terapias"]}"></span> Centro de terapias</div>
    <div class="item"><span class="muestra" style="background:${COLORES_CATEGORIA["profesional"]}"></span> Profesional</div>
    <hr/>
    <div><em>Colores por especialidad (etiquetas):</em></div>
    ${ESPECIALIDADES.map(e => `<div class="item"><span class="muestra" style="background:${COLORES_ESPECIALIDAD[e]}"></span> ${e}</div>`).join("")}
  `;
}
...
async function cargarDatos() {
  try {
    ...
    el.estadoBD.innerHTML = 'Base de datos: fallback embebido (no se pudo leer <strong>data.json</strong>).';
  }
}
